...
def _render_freshness(snap: OpsSnapshot) -> None:
    ...
    if status == "empty":
        st.info("还没有可读的日报。等同步上来后，这里会显示是否最新。")
    elif status == "failed":
        st.warning(
            "同步失败时会一直显示旧快照或演示数据。"
            "可在下方「这是什么意思？」了解如何处理。"
        ...
        st.caption("「截至时间旧」表示快照过期；等本机同步新日报后会更新。")
...
def _render_fills_detail(snap: OpsSnapshot, *, heading: bool = True) -> None:
    """Trade detail — survives flat positions (closed fills still listed)."""
    if heading:
        st.markdown(
    ...
    if not rows:
        ...
        if a + b == 0:
            st.markdown(
                ...
                "（无事件日 0 笔是正常的；可看「最近在干什么」确认是否跑过）。</div>",
                unsafe_allow_html=True,
            )
        else:
            st.info(
...
def _render_lane_a_tech_watchlist(snap: OpsSnapshot) -> None:
            ...
            rows.append(
                {
                    "标的": n.get("symbol"),
                    "score": n.get("score"),
                    "理由": n.get("reason") or "; ".join(n.get("reasons") or []),
                }
            )
    ...
    with st.expander("原始面板 JSON", expanded=False):
        st.json(wl)
...
def _data_one_liner(snap: OpsSnapshot) -> str:
    ...
    if snap.data_mode == "demo_fixtures":
        return f"数据：演示数据（示例），截至 {as_of}"
    if snap.hosting_mode == "cloud":
        return f"数据：云端已同步的虚拟盘日报（截至 {as_of}）"
    if snap.data_mode == "empty":
        return "数据：还没有可读的日报"
    return f"数据：本机虚拟盘日报（截至 {as_of}）"


def _render_secondary(snap: OpsSnapshot) -> None:
    """Independent human-titled detail blocks (no aggregate secondary label)."""
    if snap.last_halts:
        st.warning(
    ...
    with st.expander("最近成交", expanded=False):
        ...
        with st.expander("成交明细", expanded=False):
            _render_fills_detail(snap, heading=False)

    with st.expander("练兵进度", expanded=False):
        _render_drill(snap)

    if snap.events or snap.last_halts:
        with st.expander("最近异常 / 停手", expanded=False):
            _render_events(snap)


def _render_ops_home() -> bool:
    """Render investment dashboard home. Returns whether browser auto-refresh is on."""
    ...
    if snap.data_mode == "demo_fixtures":
        ...
        st.info("还没有可读的日报。等日报同步上来后，这里会换成实况。")
    elif snap.hosting_mode == "cloud":
        st.caption("当前为云端只读快照。此页不会连你电脑上的行情软件。")
    else:
        st.caption("当前为本机预览 · 虚拟盘实况。")
...
- **今天安全吗**：一眼结论 + 有没有要留意的事。
- **今天选了谁**：今天趋势路线打算交易谁 / 不交易。
- **最近一天赚亏多少**：最近一个交易日两条路线合计。
- **双路今天怎样**：A 是否在找机会/持仓/停手；B 是否有财报事件或已部署。
- **持仓是什么**：标的、数量、均价、名义金额、浮动盈亏（日报有则显示）。
- **盈亏走势 / 最近成交 / 练兵进度**：可折叠查看细节。
- **演示数据**：样例；接上真实日报后会换成实况。
- **云端看板**：只读已同步快照，**不会**连你电脑上的行情/交易软件。
...
def main() -> None:
    ...
    tab_ops, tab_lab = st.tabs(["投资看板", "高级调试"])
    with tab_ops:
        auto_ops = _render_ops_home()
    with tab_lab:
        _render_lab()
